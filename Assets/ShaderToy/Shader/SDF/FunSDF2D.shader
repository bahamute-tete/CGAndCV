...
//////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            fixed4 frag (v2f i) : SV_Target
            {
			    ...
			    col.rgb =1- sign(dt)*float3(0.7,0.33,0.1)*float3(sin(dt-_Time.y),sin(dt+_Time.y),sin(dt+_Time.y*0.5));
				col *= 1.0 - exp(-2.0*abs(dt));
				//float t = min(fmod(dt*100-_Time.y*5,10),1);
				//float fac = sin(1-t)*sin(t);
				//col.rgb *=0.2+0.3*fac; 
				col.rgb *=0.2+0.3* sin(250*dt -_Time.y*3);
				col.rgb +=lerp( col.rgb, float3(1,1,1), smoothstep(0.1,0.09,abs(dt)));
				////////////////////////
                return col;
            }
            ENDCG
