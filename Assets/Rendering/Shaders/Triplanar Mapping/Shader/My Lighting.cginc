...
InterpolatorsVertex MyVertexProgram (VertexData v) {
	...
	ComputeVertexLightColor(i);
	   ...
	   #if defined(PARALLAX_SUPPORT_DYNAMIC_BATCHING)
		//Unity doesn't normalize the normal and tangent vectors of batched geometry, 
		//after combining them in a single mesh.
...
float2 ParallaxRaymarching (float2 uv, float2 viewDir) {
	...
	float2 uvDelta = viewDir * (stepSize * _ParallaxStrength); //one Step
	...
	float  prevDifference = preStepHeight - prevSurfaceHeight;
	float difference = surfaceHeight -stepHeight;

	// v1= a+(b-a)t v2=c+(d-c)t ==> t = (a-c)/((a-c)+(d-b)) where A= a-c B=d-b
...
void ApplyParallax (inout Interpolators i) {
	...
	//parallax mapping with offset limiting only use for small value
	// #if defined(_PARALLAX_MAP)
	// 	i.tangentViewDir = normalize(i.tangentViewDir);

	// 	float height = tex2D(_ParallaxMap,i.uv.xy).g;
	// 	//high areas get pulled upwards
	// 	//low areas move downward as well,
	// 	height -=0.5;
	// 	height *= _ParallaxStrength;
	// 	//When looking straight at the surface, at a 90° angle, 
	// 	//the view direction in tangent space is equal to the surface normal (0, 0, 1),
	// 	// which results in no displacement.
	...
	// #endif
		...
		//scale the view direction vector so it's Z component becomes 1
		//The standard shader alleviates this by adding a bias to the Z component, which is 0.42, 
		//so it never gets close to zero
