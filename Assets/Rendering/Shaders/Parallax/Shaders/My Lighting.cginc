...
float2 ParallaxRaymarching (float2 uv, float2 viewDir) {
	float2 uvOffset = 0;
	float stepSize = 0.1;
	float uvDelta = viewDir * (stepSize * _ParallaxStrength); //one Step
...
void ApplyParallax (inout Interpolators i) {
	...
	// #endif
		...
		//so it never gets close to zero
			...
			#if !defined(_PARALLAX_BIAS)
				#define _PARALLAX_BIAS 0.42
			#endif
		i.tangentViewDir.xy /= (i.tangentViewDir.z+_PARALLAX_BIAS) ;
		#endif
		...
		i.uv.xy +=uvOffset;
		i.uv.zw += uvOffset * (_DetailTex_ST.xy/_MainTex_ST.xy);//consider detail texture uv scale
	#endif
}
